...
class DataGenerator():
    ...
    def shorten_past_data(self, past_data, all_generated_ids, token_count, start_location, duplicate_num=1):
        ...
        new_data["token_ids"] = torch.stack([torch.tensor(ids[:start_location], dtype=torch.long) for ids in all_generated_ids for _ in range(duplicate_num)]).to(self.model.device)
    ...
    def shorten_past_data(self, past_data, all_generated_ids, token_count, start_location, duplicate_num=1):
        ...
        new_data = {
            ...
            "token_ids": torch.stack([
                torch.tensor(ids[:start_location], dtype=torch.long)
    ...
    def generate_sample_data(self, sample_idx=0, prompt_lang='ch'):
        if prompt_lang=='ch':
        ...
        stats0 = generate_node_stats(idx=None, if_switch=None, if_natural=None, if_en_to_ch=None, switch_position=None, score=score0, answer=answer0, token_count=token_counts[0], gt_answer=gt_answer, heuristic=None)
        node0 = Node(stats=stats0)
        all_nodes = [node0]

        
        # Step 1: For all natural switches (excluding position 0), apply unswitch constraints
        natural_switch_positions = np.nonzero(all_switches_np[0])[0].tolist()
        natural_switch_positions = [pos for pos in natural_switch_positions if pos > 0 and pos<len(all_switches_np[0])-1]
        ...
        if num_synthetic > 0:
            ...
            valid_positions = np.array([pos for pos in valid_positions if pos > 0 and pos < len(all_switches_np[0])-1])
            ...
            if len(valid_positions) > 0:
                ...
                sorted_idx = np.argsort(-heuristics_at_valid[mask])
        ...
        for i in range(0, len(paired), batch_size):
            ...
            generated_texts, token_counts_, all_generated_ids_, all_masks_np_, all_switches_np_, all_heuristics_, _, activations_np = generate_constrained_response_batch(self.model, inputs, self.tokenizer, self.vocab, self.token_class, self.cfg.dataset.max_tokens, batch_modes, batch_locations, do_sample = self.cfg.dataset.do_sample, temperature = self.cfg.dataset.temperature, top_p = self.cfg.dataset.top_p, \
                                                                                                                                   past_data=past_data_truncated, save_past_data=False, heuristic_measure=None, ch_composite_tokens=self.ch_composite_tokens, layer_num=self.cfg.dataset.layer_num)
            ...
            for b in range(len(batch_modes)):
                ...
                stats = generate_node_stats(idx=start_idx+b, if_switch=if_switch, if_natural=if_natural, if_en_to_ch=if_en_to_ch, switch_position=batch_locations[b], score=score, answer=answer, token_count=token_counts_[b], gt_answer=gt_answer, heuristic=all_heuristics[0][batch_locations[b]].item())
    ...
    def generate_all_data(self, data_range=[0,10], prompt_lang='ch'):
        for sample_idx in tqdm(range(data_range[0], data_range[1])):
            try:
                self.generate_sample_data(sample_idx, prompt_lang)
            except:
