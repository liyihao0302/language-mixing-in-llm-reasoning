...
def generate_constrained_response_batch(
    ...
    with torch.no_grad():
        # shape of logits: [batch_size, seq_len, vocab_size]
        if past_data is None or past_data['key_values'] is None:
            current_locations = torch.tensor([0] * batch_size, device=device)
        ...
        # -------------------------------------------------------------------------
            ...
            if past_data is not None and past_data['token_ids'] is not None:
                #import pdb; pdb.set_trace()
                current_max = current_locations.max(dim=0).values  # shape: (2,)
                mask = current_max < torch.tensor(locations).to(current_max.device)
                if current_locations.max() <= past_data['token_ids'][0].shape[-1]-1:
                    
                    all_chosen[mask] = past_data['token_ids'][mask, current_locations.max()]
            ...
            start_location = past_data['start_location']
